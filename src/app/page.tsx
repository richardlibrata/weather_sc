...
export default function Home() {
  ...
  const { isLoading, error, data, refetch } = useQuery<WeatherData>("repoData", async () => {
      const { data } = await axios.get(
        `https://api.openweathermap.org/data/2.5/forecast?q=${place}&appid=${process.env.PUBLIC_KEY}&cnt=56`
      );
      return data;
    }
  );
  ...
  const firstDataForEachDate = uniqueDates.map((date) => {
    return data?.list.find((entry) => {
      ...
      return entryDate === date && entryTime >= 3;
    });
  });
  ...
  return (
    <div className="flex flex-col gap-4 bg-green-100 min-h-screen">
      ...
      <section className="space-y-4">
          ...
          <Container className="gap-10 px-6 items-center">
            ...
            <div className="flex gap-10 sm:gap-16 overflow-x-auto w-full justify-between pr-3">
              ...
              <div key={i} className="flex flex-col justify-between gap-2 items-center text-xs font-semibold">
                ...
                <WeatherPic iconName={getDayorNight(d.weather[0].icon, d.dt_txt)}/>
        ...
        <div className="flex gap-4">
          <Container className="w-fit justify-center flex-col px-4 items-center">
            ...
            <WeatherPic iconName={getDayorNight(firstData?.weather[0].icon ?? "", firstData?.dt_txt ?? "")}/>
          </Container>
          <Container className="bg-purple-200/80 px-6 gap-4 justify-between overflow-x-auto">
          <WeatherDetail
            visibility={metersToKilometers(firstData?.visibility ?? 10000)}
            airPressure={`${firstData?.main.pressure} hPa`}
            humidity={`${firstData?.main.humidity}%`}
            sunrise={format(
              fromUnixTime(data?.city.sunrise ?? 1727214482), "HH:mm")}
            sunset={format(
              fromUnixTime(data?.city.sunset ?? 1727258119), "HH:mm")}
      ...
      <section className="flex w-full flex-col gap-4">
        <p className="text-2xl">Forecast Next 6 Days</p>
